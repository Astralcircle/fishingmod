...
if CLIENT then
	...
	function SWEP:Initialize()
		if not self.hasMentioned and not game.SinglePlayer() and self:GetOwner() == LocalPlayer() then
			chat.AddText(Color(0, 255, 0), "[Fishing Mod]", Color(255, 255, 255), ": Чтобы открыть меню, забиндите команду fishing_mod_menu на любую кнопку.")
	...
	function SWEP:Deploy()
		...
		if not IsValid(self.fishing_rod) then
			...
			elseif not self.hasMentioned and game.SinglePlayer() and IsValid(self:GetOwner()) then
				self:GetOwner():ChatPrint("[Fishing Mod]: Чтобы открыть меню, забиндите команду fishing_mod_menu на любую кнопку.")
				self.hasMentioned = true
			end
