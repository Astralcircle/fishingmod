...
if CLIENT then
	...
	function SWEP:Initialize()
		if not self.hasMentioned and not game.SinglePlayer() and self:GetOwner() == LocalPlayer() then
			chat.AddText(Color(0, 255, 0), "[Рыбалка]", Color(255, 255, 255), ': Нажмите "B" по стандарту чтобы открыть меню рыбалки. \nЕсли вы привязали "B" к чему-то другому: bind <кнопка> fishing_mod_menu.')
	...
	function SWEP:Deploy()
		...
		if not IsValid(self.fishing_rod) then
			...
			elseif not self.hasMentioned and game.SinglePlayer() and IsValid(self:GetOwner()) then
				self:GetOwner():ChatPrint('[Рыбалка]: Нажмите "B" по стандарту чтобы открыть меню рыбалки. \nЕсли вы привязали "B" к чему-то другому: bind <кнопка> fishing_mod_menu.')
				self.hasMentioned = true
			end
