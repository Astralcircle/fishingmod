...
function PANEL:Init()
	...
	upgrades_button:SetText("Апгрейды")
...
function PANEL:Init()
	...
	self.seagull_deter:SetType("Отпугиватель чаек:", "seagull_deter", "seagull_deter", fishingmod.SeagullDeterPrice)
...
function PANEL:Init()
	...
	save_button.DoClick = function()
		if fishingmod.SaveUIColors then
			...
			chat.AddText(col_green, "[Fishing Mod]", col_white, ": Цвета сохранены!")
		end
	end
	...
	defaults_button.DoClick = function()
		if fishingmod.DefaultUIColors then
			fishingmod.ColorTable = fishingmod.DefaultUIColors()
			chat.AddText(col_green, "[Fishing Mod]", col_white, ": Цвета сброшены до стандартных!")
		end
	end
...
function fishingmod.UpdateSales()
	for key, bait in pairs(fishingmod.BaitTable) do
		...
		if IsValid(bait.icon) then
			bait.icon:SetToolTip(sale .. "\nВам нужен уровент " .. levelrequired .. " или выше чтобы использовать эту наживку.")
